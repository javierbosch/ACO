...
public class ACO implements Runnable{
    ...
    public ACO(int id, double[][] dist, double alpha, double beta, double rho, double Q, int sigma, int numAnts, int maxIterations, String output_file, int totalColonies){
        this.dist = dist;
        this.alpha = alpha;
        this.beta = beta;
        this.rho = rho;
        ...
        this.id = id;
        try {
            ACO.writer = new FileWriter(output_file,true);
        } catch (IOException e) {
        ...
        ACO.totalColonies = totalColonies;
        ACO.totalDone = 0;
    }
    ...
    public void run(){
        ...
        if (totalDone == totalColonies) {
            try {
                writer.close();
                System.out.printf("GOOD JOB YOU FINISHED ALL %d COLONIES, MULTITHREADING!!!\n", totalColonies);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        }
    ...
    void UpdateRho(int i){
        if (i > 2){
            rho = 1 - (4 / (Math.pow(i, 0.784) + 2));
        }
    }
    ...
    void WriteState() {
        try {
            String line =
                ...
                this.bestLength + "," +
                this.currBestLength + "," +
                Arrays.toString(this.bestPath).replace(",", " -") + "\n";
            writer.write(line);
            writer.flush();
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    private double[] MoveProb(int cityX, boolean[] visited)
    {
        ...
        for (int i = 0; i <= attractiveness.length - 1; i++)
        {
            ...
            {
                attractiveness[i] = Math.pow(pheromones[cityX][i], alpha) * Math.pow((1.0 / Distance(cityX, i)), beta);
                if (attractiveness[i] < 0.0001)
                {
                    attractiveness[i] = 0.0001;
                }
                else if (attractiveness[i] > (Double.MAX_VALUE / (numCities * 100)))
                {
                    attractiveness[i] = Double.MAX_VALUE / (numCities * 100);
                }
            }
    ...
    private double DeceptiveDistance(int cityX, int cityY)
    {
        Random rnd = new Random();
        return 2 * dist[cityX][cityY] * rnd.nextDouble();
    }
    ...
    private void UpdatePheromones()
    {
        for (int i = 0; i <= pheromones.length - 1; i++)
        {
            for (int j = i + 1; j <= pheromones[i].length - 1; j++) // change if graph is directed
            {
                double sum = 0.0;
                ...
                pheromones[j][i] = pheromones[i][j]; // change if graph is directed
            }
        }
    }
